...
export default function ExamAttemptPage({
  ...
  const [essaySaveTimeout, setEssaySaveTimeout] = useState<any>(null);
  ...
  const handleEssayChange = (questionId: string, value: string) => {
    ...
    const updatedQuestions = questions.map((q) => {
      if (q.questionId === questionId) {
        return { ...q, essayAnswer: value };
      }
      return q;
    });
    setQuestions(updatedQuestions);

    // B. Queue background debounced autosave
    debounceAutosaveEssay(questionId, value);
  };

  const debounceAutosaveEssay = (questionId: string, value: string) => {
    setSaving(true);
    if (essaySaveTimeout) {
      clearTimeout(essaySaveTimeout);
    }
    const timeout = setTimeout(async () => {
      try {
        const res = await fetch(`/api/candidate/attempts/${attempt!.id}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ questionId, essayAnswer: value }),
        });
        const data = await res.json();
        if (!res.ok) {
          if (data.expired) {
        ...
        console.error('Autosave essay network error');
      } finally {
        setSaving(false);
      }
    }, 1000);
    setEssaySaveTimeout(timeout);
  };
        ...
        <div className="bg-white dark:bg-[#0b0f19] border border-slate-200 dark:border-slate-800/80 rounded-xl p-4 shadow-lg h-fit space-y-3 lg:sticky lg:top-4">
          ...
          <div className="grid grid-cols-5 gap-2.5">
            {questions.map((q, index) => {
              ...
              const isAnswered = q.questionType === 'essay'
                ? (q.essayAnswer !== null && q.essayAnswer !== undefined && q.essayAnswer.trim() !== '')
                : (q.selectedOption !== null);
            ...
            {currentQuestion.questionType === 'essay' ? (
              <div className="space-y-2 pt-1">
                <label className="block text-xs font-semibold text-slate-400 font-sans">
                  Ketik jawaban Anda di sini secara lengkap:
                </label>
                <textarea
                  ...
                  disabled={submitLoading}
                  rows={6}
                  placeholder="Ketik jawaban isian Anda..."
                  className="w-full p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 focus:border-violet-400 dark:focus:border-[#00d8f6] rounded-xl text-xs text-slate-850 dark:text-white placeholder-slate-400 dark:placeholder-slate-600 focus:outline-none transition-colors resize-y min-h-[150px]"
                />
              </div>
            ) : (
